...
def evaluar_en_test(conn, tabla: str, study: optuna.Study, mes_test: str, 
    ...
    Args:
        ...
        es_test_final: Si es True, es el último test (no limpia memoria de modelos)
    ...
    # Estadísticas usando NumPy puro
    # Ordenar por probabilidad descendente
    indices_ordenados = np.argsort(y_pred_promedio)[::-1]
    y_true_ordenado = y_test[indices_ordenados]
    ...
    if not es_test_final:
        ...
        # Es el test final, solo limpiamos lo necesario
        logger.info("Test final - preparando para entrenamiento con datos extendidos")
        del models, y_pred_futuro, pred_matrix, y_pred_promedio
        del X_train_completo, y_train_completo, X_test, y_test, train_data, test_data
        gc.collect()
...
def sincronizar_db_con_gcs():
    """
    Sube la base de datos de Optuna a GCS usando DuckDB.
    """
    local_db_dir = os.path.expanduser("~/optuna_db")
    db_file = os.path.join(local_db_dir, f"{STUDY_NAME}.db")
    ...
    try:
        ...
        # Crear conexión temporal para subir
        temp_conn = duckdb.connect(':memory:')
        temp_conn.execute("INSTALL httpfs; LOAD httpfs;")
        
        # Configurar GCS (mismo código que en 02_optimizacion.py)
        ...
        # Escribir archivo a GCS
        temp_conn.execute(f"""
            COPY (SELECT '{db_content.hex()}'::BLOB AS data) 
            TO '{gcs_path}' 
            (FORMAT PARQUET)
        """)
        
        temp_conn.close()
        logger.info(f"DB sincronizada con GCS: {gcs_path}")
        
    except Exception as e:
        logger.error(f"Error al sincronizar DB con GCS: {e}")
