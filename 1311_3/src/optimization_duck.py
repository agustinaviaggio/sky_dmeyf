...
def objetivo_ganancia(trial, conn, tabla: str) -> float:
    ...
    Entrena con diferentes meses según el valor de target_binario.
    ...
    # Log del tamaño del dataset
    logger.info(f"Trial {trial.number} - Registros train target=0: {(train_data['target_binario'] == 0).sum()}")
    logger.info(f"Trial {trial.number} - Registros train target=1: {(train_data['target_binario'] == 1).sum()}")
